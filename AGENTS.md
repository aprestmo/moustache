...
- `dist/` is committed as a safety net. CI lives at **`.gitea/workflows/build.yml`** (Gitea Actions — it rebuilds and commits `dist/` on push to `main` when `src/`, `public/`, or build config change, with `[skip ci]`). Its final step connects to `root@89.167.31.44:22`, runs the exact target commit through `auto-deploy.sh`, and fails if SSH/Git/manifest verification fails. It requires the Gitea Actions secrets `MOUSTACHE_DEPLOY_SSH_KEY` and `MOUSTACHE_DEPLOY_KNOWN_HOSTS`. It only runs if a Gitea Act runner is registered **and** repo Actions are enabled (README → CI section); otherwise use webhook/cron or build on the server with `./deploy.sh`. Never hand-edit `dist/`.
...
Theme runs on Coolify/Docker at kampbart.com, mounted as a Docker volume; repo host is **Gitea** (git.attityd.no), not GitHub. Flow: push to `main` → Gitea Action (for frontend changes) SSHes to `root@89.167.31.44:22`, verifies the exact target commit, pulls the committed `dist/`, and returns the real command result to the Action; the push webhook uses `action=moustache_deploy` and host cron remains a fallback for PHP/push changes but refuses unapproved frontend source. Manual fallback is `git pull origin main` on the server → `./deploy.sh` when `src/`/`public/` changed, or explicitly `MOUSTACHE_DEPLOY_ALLOW_SERVER_BUILD=1 bash auto-deploy.sh` (PHP, `js/`, `acf-json/` are picked up by the pull alone). The `.gitea/workflows/build.yml` CI needs a registered Gitea Act runner and the `MOUSTACHE_DEPLOY_SSH_KEY` / `MOUSTACHE_DEPLOY_KNOWN_HOSTS` repository secrets; webhook/cron must not depend on it. Webhook needs `MOUSTACHE_WEBHOOK_SECRET` in `wp-config.php` matching the Gitea webhook secret (setup in README → Auto-deploy). Note: prod has the whole REST API disabled via an external `rest_authentication_errors` filter (`rest_disabled`) — not in this repo; a REST-based route will 403 there, which is why the webhook uses admin-ajax. Full steps, the volume path, and CI requirements are in `README.md`.

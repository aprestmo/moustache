...
- `bash auto-deploy.sh` — server-side: fetch `origin/main`, ff-merge, and deploy only approved changes; CI uses committed `dist/`, while ordinary webhook/cron runs refuse unapproved frontend source (lock-protected). Manual server builds require `MOUSTACHE_DEPLOY_ALLOW_SERVER_BUILD=1`.
...
- `dist/` is committed as a safety net. CI lives at **`.gitea/workflows/build.yml`** (Gitea Actions — it rebuilds and commits `dist/` on push to `main` when `src/`, `public/`, or build config change, with `[skip ci]`). Its final step calls the signed production deploy endpoint synchronously, so a green frontend Action means the server pulled the committed `dist/`; it requires a Gitea Actions secret named `MOUSTACHE_WEBHOOK_SECRET`. It only runs if a Gitea Act runner is registered **and** repo Actions are enabled (README → CI section); otherwise use webhook/cron or build on the server with `./deploy.sh`. Never hand-edit `dist/`.
...
Theme runs on Coolify/Docker at kampbart.com, mounted as a Docker volume; repo host is **Gitea** (git.attityd.no), not GitHub. Flow: push to `main` → Gitea Action (for frontend changes) makes a signed synchronous request to `POST /wp-admin/admin-ajax.php?action=moustache_deploy_ci`, which verifies the exact target commit, pulls the committed `dist/`, and returns success to the Action; the push webhook uses `action=moustache_deploy` and host cron remains a fallback for PHP/push changes but refuses unapproved frontend source. Manual fallback is `git pull origin main` on the server → `./deploy.sh` when `src/`/`public/` changed, or explicitly `MOUSTACHE_DEPLOY_ALLOW_SERVER_BUILD=1 bash auto-deploy.sh` (PHP, `js/`, `acf-json/` are picked up by the pull alone). The `.gitea/workflows/build.yml` CI needs a registered Gitea Act runner and the `MOUSTACHE_WEBHOOK_SECRET` repository secret for the final deploy step; webhook/cron must not depend on it. Webhook needs `MOUSTACHE_WEBHOOK_SECRET` in `wp-config.php` matching the Gitea secret (setup in README → Auto-deploy). Note: prod has the whole REST API disabled via an external `rest_authentication_errors` filter (`rest_disabled`) — not in this repo; a REST-based route will 403 there, which is why the webhook uses admin-ajax. Full steps, the volume path, and CI requirements are in `README.md`.

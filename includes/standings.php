...
/**
 * Standings service: REST endpoint, cache helpers, season gate, Tools → Standings.
 *
 * Deliberately theme-independent (no template/ACF dependencies) so it can
 * outlive this theme when the Astro front end (moustache-v7) takes over.
 *
 * Loading — supported paths (never both, functions.php guards on
 * function_exists('fetch_standings_data')):
 *   1. Preferred: mu-plugins/moustache-standings.php (a 3-line loader stub,
 *      symlinked or copied to wp-content/mu-plugins/ on deploy) requires
 *      this file early, before the theme loads.
 *   2. Fallback: functions.php requires this file when the loader is absent.
 */

/**
 * ─── SEASON CONFIGURATION — UPDATE ANNUALLY (both values, right here) ───
 */

/**
 * Last day of the current season; standings stop being fetched after this date.
 * UPDATE ANNUALLY (e.g. '2027-12-31' at the next year rollover).
 */
const MOUSTACHE_SEASON_END_DATE = '2026-12-31';

/**
 * `tournament` taxonomy slug of the current season's league.
 * UPDATE ANNUALLY (e.g. 'uteserie-2027' at the next year rollover).
 */
const MOUSTACHE_SEASON_TOURNAMENT_SLUG = 'uteserie-2026';
...
{
    // Check if the current season is still active
    if (!is_season_2026_active()) {
        error_log('Standings: season ended (' . MOUSTACHE_SEASON_END_DATE . '), no longer fetching data');
        return false;
    }
...
/**
 * Check if the current season is still active based on fixtures
 *
 * Uses MOUSTACHE_SEASON_END_DATE as the hard cutoff and looks for future
 * matches in the MOUSTACHE_SEASON_TOURNAMENT_SLUG tournament.
 *
 * @return bool True if season is active, false if ended
 */
function is_season_2026_active(): bool
{
    // First check the simple date cutoff
    $current_date = current_time('Y-m-d');
    if ($current_date > MOUSTACHE_SEASON_END_DATE) {
        return false;
    }

    // Check if there are any future matches in the current season's tournament
    ...
    $future_matches = new WP_Query([
        ...
        'tax_query' => [
            [
                'taxonomy' => 'tournament',
                'field' => 'slug',
                'terms' => MOUSTACHE_SEASON_TOURNAMENT_SLUG,
            ],
        ],
        'meta_query' => [
            'relation' => 'OR',
...
function standings_admin_page() {
    ...
    <div class="wrap">
        ...
        <h2>Season Status</h2>
        <?php
        $is_active = is_season_2026_active();
        echo '<p>Season ' . esc_html(MOUSTACHE_SEASON_TOURNAMENT_SLUG) . ' active: ' . ($is_active ? 'YES' : 'NO') . '</p>';
        echo '<p>Season cutoff date: ' . esc_html(MOUSTACHE_SEASON_END_DATE) . '</p>';
